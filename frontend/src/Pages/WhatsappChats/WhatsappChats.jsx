...
export default function WhatsappChats() {
  ...
  useEffect(() => {
    ...
    if (
      !socket ||
      !currentCustomerRef.current?.mobile ||
      !currentCustomerRef.current._id
    ) {
      console.log("returing");
      return;
    }

    socket.emit("join_customer_chat", {
      //one time
  ...
  const handleOnCustomerSelect = (customer) => {
    ...
    if (
      currentCustomerRef.current &&
      customer._id === currentCustomerRef.current._id &&
      chatUI === "open"
    )
      return;
  ...
  return (
    <div className="h-[calc(100vh-64px)] grid grid-cols-12 bg-gray-300/40 rounded-2xl gap-9 fixed">
      ...
      <div className="col-span-4 bg-white rounded-xl shadow-sm p-4 m-2">
        <CustomerPicker
          items={customers}
          onSelect={handleOnCustomerSelect}
          selected={currentCustomerRef}
        />
      </div>
